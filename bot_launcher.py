...
def _duplicate_text(r: dict) -> str:
    return (
        ...
        f"📊 الحالة: {STATUS_LABELS.get(r.get('status'), legacy_bot.STATUS_MAP.get(r.get('status'), '—'))}\n"
        f"💬 الرد: {'✅ يوجد رد' if _reply_exists(r) else '❌ لا يوجد رد'}\n\n"
        "لم يتم إنشاء طلب جديد ولم يتم تغيير الطلب الموجود."
    )
