...
/**
 * Update dashboard statistics and chart - مع فحوصات للعناصر
 */
function updateDashboard(requests) {
    ...
    // تحديث الأرقام مع فحص وجود العناصر
...
function editRequest() {
    ...
    const reqAuthority = document.getElementById('reqAuthority');
    const reqDate = document.getElementById('reqDate');
    const reqStatus = document.getElementById('reqStatus');
    const submitText = document.getElementById('submitButtonText');

    if (reqId) reqId.value = currentSelectedRequest.reqId || '';
    if (reqTitle) reqTitle.value = currentSelectedRequest.title || '';
    if (reqDetails) reqDetails.value = currentSelectedRequest.details || '';
    if (reqAuthority) reqAuthority.value = currentSelectedRequest.authority || '';
    if (reqDate) reqDate.value = currentSelectedRequest.submissionDate || '';
    if (reqStatus) reqStatus.value = currentSelectedRequest.status || 'execution';
    if (submitText) submitText.textContent = 'تحديث الطلب';
        ...
        <body>
            ...
            <div style="margin-top: 40px; padding: 25px; background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 15px; text-align: center; color: white;">
                <div style="display: flex; align-items: center; justify-content: center; gap: 15px; margin-bottom: 10px;">
                    <div style="width: 40px; height: 40px; background: rgba(255,255,255,0.2); border-radius: 10px; display: inline-flex; align-items: center; justify-content: center; font-size: 20px;">
                        💻
                    </div>
                    <div>
                ...
                <div style="font-size: 12px; opacity: 0.9; margin-top: 10px;">
                    <a href="https://www.facebook.com/en.mohamed.nasr" style="color: white; text-decoration: none;">
